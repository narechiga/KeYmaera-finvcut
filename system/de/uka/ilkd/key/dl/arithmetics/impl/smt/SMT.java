...
//		Term parsedTerm = String2TermConverter.convert(res, nss);
		...
		String smtIn = "(set-logic QF_NRA)\n";
		smtIn += input.getVariableList() + "\n";
		...
		try {
			...
			/* intercept the smtIn to filter out mishandled instances of E */
			//System.out.println("Intercepting...");
			//System.out.println("\u001B[33m"+ "\u001B[1m" + "WARNING: " + "\u001B[0m" + "KeYmaera mishandles E. Making a best-effort attempt to fix this in dReal interface.");
			smtIn = smtIn.replace("(declare-fun Edollarclosureuscore0 () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun Edollarclosureuscore () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun Edollarclosure () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun Edollarclosure1 () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun Edollarclosure2 () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun E () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun E1 () Real)\n", "");
			smtIn = smtIn.replace("(declare-fun E2 () Real)\n", "");

			smtIn = smtIn.replace("Edollarclosureuscore0", "(exp 1)");
			smtIn = smtIn.replace("Edollarclosureuscore", "(exp 1)");
			smtIn = smtIn.replace("Edollarclosure", "(exp 1)");
			smtIn = smtIn.replace("Edollarclosure1", "(exp 1)");
			smtIn = smtIn.replace("Edollarclosure2", "(exp 1)");
			smtIn = smtIn.replace("E", "(exp 1)");
			smtIn = smtIn.replace("E1", "(exp 1)");
			smtIn = smtIn.replace("E2", "(exp 1)");

			System.out.println("===============================================================================================================================");
			System.out.println( smtIn );
			System.out.println("===============================================================================================================================");
			...
			String runString = Options.INSTANCE.getZ3Binary().getAbsolutePath() + " " + "--precision=0.000001 " + inputFile.getAbsolutePath();
			process = Runtime.getRuntime().exec( runString );
			...
			while((line = b.readLine()) != null) {
				...
				/*if(line.indexOf(start) != -1) {
					...
					if(line.equals("unsat") ) {
						return TermBuilder.DF.tt();
					} else if( line.equals("sat") || line.equals("unknown") ) {
						return TermBuilder.DF.ff();
					}
				/*} else {
