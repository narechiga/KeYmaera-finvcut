 ...
 */
public class BarrierDiffInd extends AbstractDLMetaOperator {

    public static final Name NAME = new Name("#BarrierDiffInd");

    public BarrierDiffInd() {
        super(NAME, 1);
    }
    ...
    public Term calculate(Term term, SVInstantiations svInst, Services services) {
        try {
            return barrierDiffInd(term.sub(0), services);
    ...
    public Term barrierDiffInd(Term term, Services services) throws SolverException {
        ...
        if (term.op() == Modality.BOX
            ...
            try {
				...
				Term bdiffInd;
				// I don't think I want to trust this thing
				/*if(DLOptionBean.INSTANCE.isUseODEIndFinMethods()) {
				    bdiffInd = MathSolverManager.getCurrentODESolver()
				            .strictDiffInd(sys, post, services);
				} else {*/
				    bdiffInd = DerivativeCreator.strictDiffInd(sys, post, services);
                    if(decl != null) {
                        // lookup variables
                        ...
                        for (QuantifiableVariable var : vars) {
                           bdiffInd = TermBuilder.DF.all(var, bdiffInd);
                        }
                    }
				    bdiffInd = TermBuilder.DF.imp(sys.getInvariant(services), bdiffInd);
				/*}*/
